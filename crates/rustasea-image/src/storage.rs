//! Global storage slot used by the `store` terminal and the queued transform.
//!
//! The crate follows the framework's process-wide-slot pattern (ADR-0007):
//! [`Image::set_storage`](crate::Image::set_storage) installs one
//! `Arc<dyn Storage>` that [`store`](crate::ImageBuilder::store) and
//! [`ImageTransformJob`](crate::ImageTransformJob) read from. Tests clear it
//! with [`Image::clear_storage`](crate::Image::clear_storage) so runs stay
//! isolated.
...
/// Return the installed disk or a typed [`ImageError::NotConfigured`].
pub(crate) fn get() -> Result<Arc<dyn Storage>> {
    ...
    slot.clone()
        .ok_or_else(|| ImageError::NotConfigured("storage (call Image::set_storage)".to_string()))
}

/// Write `bytes` to `key` on the installed disk.
///
/// Awaits the async [`Storage`] write directly; used by
/// [`store_async`](crate::ImageBuilder::store_async) and the queued
/// [`ImageTransformJob`](crate::ImageTransformJob).
pub(crate) async fn put_bytes_async(key: &str, bytes: &[u8]) -> Result<()> {
    let storage = get()?;
    storage
        .put(key, bytes)
        .await
        .map_err(|e| ImageError::Storage {
            key: key.to_string(),
            message: e.to_string(),
        })
}
...
///
/// Bridges the async [`Storage`] trait for the synchronous `store` terminal.
/// The strategy depends on the ambient Tokio context so it never panics and
/// never deadlocks:
///
...
/// [`store_async`](crate::ImageBuilder::store_async) or a queued
/// [`ImageTransformJob`](crate::ImageTransformJob).
pub(crate) fn put_bytes(key: &str, bytes: &[u8]) -> Result<()> {
    let storage = get()?;
    let future = storage.put(key, bytes);
    let outcome = drive_blocking(future).map_err(|e| ImageError::Storage {
        key: key.to_string(),
        message: e.to_string(),
    })?;
    outcome.map_err(|e| ImageError::Storage {
        key: key.to_string(),
        message: e.to_string(),
    })
}

/// Read `key` from the installed disk (used by the queued transform).
#[cfg(feature = "queue")]
pub(crate) async fn get_bytes_async(key: &str) -> Result<Vec<u8>> {
    let storage = get()?;
    storage.get(key).await.map_err(|e| ImageError::Storage {
        key: key.to_string(),
        message: e.to_string(),
    })
}
        ...
        Ok(handle) => match handle.runtime_flavor() {
            ...
            _ => std::thread::scope(|scope| {
                scope
                    ...
                    .join()
                    .map_err(|_| std::io::Error::other("image storage worker thread panicked"))?
            }),
        },
        Err(_) => {
            let runtime = Builder::new_current_thread().enable_all().build()?;
