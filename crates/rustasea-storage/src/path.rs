...
mod tests {
    ...
    fn lexical_parent_dir_is_traversal() {
        let root = Path::new("/tmp/rustasea-store");
        let err = confine_path(root, Path::new("../../etc/passwd")).unwrap_err();
        assert_eq!(
            err.0,
            "candidate \"../../etc/passwd\" escapes root \"/tmp/rustasea-store\""
        );
    }
    ...
    fn symlinked_parent_outside_root_is_traversal() {
        ...
        // it to `root.join("escape/rustasea-escape")` would write into `/etc`.
        let base =
            std::env::temp_dir().join(format!("rustasea-path-symlink-{}", std::process::id()));
        ...
        let outcome = confine_path(&root, &link.join("rustasea-escape"))
    ...
    fn missing_path_under_safe_symlink_is_confined() {
        ...
        let base =
            std::env::temp_dir().join(format!("rustasea-path-symlink-in-{}", std::process::id()));
        std::fs::remove_dir_all(&base).ok();
        std::fs::create_dir_all(&base).unwrap();
