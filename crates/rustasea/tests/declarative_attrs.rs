...
//!
//! Every `rustasea-macros` attribute re-emits the decorated item unchanged and
...
//! the item's type name (`__RUSTASEA_TRIES_<Type>`); bare markers (`hidden`,
//! `failOnTimeout`, `withoutBroadcasting`, `repairToolCalls`) emit a
//! type-less `__RUSTASEA_<KEY>: bool`, so marker items live in their own
...
fn tries_emits_retry_budget() {
    assert_eq!(__RUSTASEA_TRIES_SendEmailsJob, 3);
}

#[test]
fn backoff_emits_base_delay_seconds() {
    assert_eq!(__RUSTASEA_BACKOFF_SECS_SendEmailsJob, 10);
}

#[test]
fn timeout_emits_attempt_deadline_seconds() {
    assert_eq!(__RUSTASEA_TIMEOUT_SECS_SendEmailsJob, 30);
    assert_eq!(__RUSTASEA_TIMEOUT_SECS_LongPollJob, 0);
}
...
fn usage_emits_signature_text() {
    assert_eq!(
        __RUSTASEA_USAGE_SendEmailsCommand,
...
fn help_emits_description_text() {
    assert_eq!(
        __RUSTASEA_HELP_SendEmailsCommand,
...
fn queue_and_connection_emit_names() {
    assert_eq!(__RUSTASEA_QUEUE_WelcomeEmailJob, "emails");
    assert_eq!(__RUSTASEA_CONNECTION_WelcomeEmailJob, "redis");
}
...
/// `__RUSTASEA_HIDDEN` const unique.
mod priority_marker {
    use super::hidden;
