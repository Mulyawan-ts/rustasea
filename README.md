...
RustaSea is split across two repositories, mirroring the `laravel/framework` and
`laravel/laravel` split:

- `rustasea/framework` (this repository) is the framework monorepo: 43 crates
...
- `rustasea/rustasea` is the application skeleton repository (analog
  `laravel/laravel`). It is generated by `cargo rustasea new` and holds the
