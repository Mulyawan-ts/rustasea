...
| `wasm-dioxus` / `wasm-leptos` | `dep:rustasea-inertia-adapters` (react/vue) | React→Dioxus / Vue→Leptos WASM adapters (ADR-0002) |
...
│   ├── rustasea-scaffold/         # M5/M6 — starter-kit variants (blade/react/vue/livewire)
...
records **43 crates under `crates/` + `xtask` (44 workspace packages total)**,
