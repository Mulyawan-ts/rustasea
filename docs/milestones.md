...
- Project scaffolder `cargo rustasea new <app> --variant {blade|react|vue|livewire}` — `crates/rustasea/src/bin/cargo-rustasea.rs:46`, `:64-74`; starter kits in `crates/rustasea-scaffold/src/variant.rs:13`.
...
- Project scaffolder `cargo rustasea new <app> --variant {blade|react|vue|livewire}` — `crates/rustasea/src/bin/cargo-rustasea.rs:46`, `:64-74`; starter kits `crates/rustasea-scaffold/src/variant.rs:13`; `cargo artisan` remains a legacy alias via `normalize_args` (`:103`).
